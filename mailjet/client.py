...
class Config:
    ...
    API_PATHS = {  # our_name:API_URL+part (http://dev.mailjet.com/email-api/v3/%s)
        ...
        # campaigns
        'bouncestatistics': 'bouncestatistics',
        'campaign': 'campaign',
        'campaignaggregate': 'campaignaggregate',
        'campaigngraphstatistics': 'campaigngraphstatistics',
        ...
        'contactstatistics': 'contactstatistics',
        'csvimport': 'csvimport',
        ...
        'messagesentstatistics': 'messagesentstatistics',
...
class Client:
    ...
    def __getattr__(self, name):
        if name.lower() in self.SUPPORTED_METHODS:
            self.method = name.lower()
            return self
        else:
            raise AttributeError

    def __call__(self, endpoint, data=None, resource_id=None, extra_headers=None,
                 timeout=None, debug=False, **kwargs):
        data, url = self._build_body_and_url(endpoint=endpoint, method=self.method,
                                                  data=data, resource_id=resource_id)
        headers = self.build_headers(extra_headers)
        req_method = getattr(requests, self.method)

        try:
            response = req_method(url, data=data, headers=headers, auth=self.auth,
                                  timeout=timeout, verify=False, stream=False)

            return self._parse_response(response, debug=debug)
    ...
    def _build_body_and_url(self, endpoint, method, data=None, resource_id=None):
        data = json.dumps(data) if data else None
        url, doc = self.config[endpoint]
    ...
    def _parse_response(self, response, debug=False):
        data = response.json()
